...
function renderIndexPage(posts) {
  // 横线区域：所有博文卡片横向排列
  const sceneCards = posts
    .map((p, i) => {
      const href = `posts/${encodeURIComponent(p.slug)}.html`;
      const dateFmt = formatDateShort(p.date);
      const num = String(i + 1).padStart(2, "0");
      return [
        `            <a class="scene-card" href="${escapeHtml(href)}" aria-label="打开博客文章：${escapeHtml(p.title)}">`,
        `              <div class="scene-card__num">${num}</div>`,
        `              <div class="scene-card__title">${escapeHtml(p.title)}</div>`,
        `              <div class="scene-card__meta">${escapeHtml(dateFmt)}</div>`,
        `            </a>`,
      ].join("\n");
    })
    .join("\n");
  ...
  return [
    ...
    "      </section>",
    "",
    '      <section class="scene" aria-label="博客列表">',
    ...
    '              id="chainPath"',
    '              class="chain__path"',
    '              d="M 0 110 L 1000 110"',
    "            />",
    "          </svg>",
    "",
    '          <div class="scene__rail" id="sceneRail">',
    '            <div class="scene__viewport" id="sceneViewport">',
    '              <div class="scene__track" id="sceneTrack">',
    sceneCards,
    "              </div>",
    "            </div>",
    "          </div>",
    "",
    '          <button class="scene__nav scene__nav--prev" aria-label="向左滚动">',
    ...
    "          </button>",
    '          <button class="scene__nav scene__nav--next" aria-label="向右滚动">',
