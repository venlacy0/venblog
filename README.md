...
- **滚动与交互** — 首页视差动效，尊重 `prefers-reduced-motion`
- **Archive 标签筛选** — 构建期输出 Tag 栏，前端无刷新筛选，支持 `?tag=` 初始化
...
项目根目录新增 `venblog.config.json`，可配置首页与文章页的大部分文案，包括：

- 站点标题、描述、页面语言
- Hero 区文案（Issue、标题、tagline）
- Archive 区标题、标签文案、状态模板、视图切换文案
- 文章页返回文案、阅读时长文案、结尾文案

修改配置后重新执行 `npm run build`；开发模式下 `npm run serve` 会自动监听并重建。
...
├── venblog.config.json  # 站点配置（首页/文章页文案）
├── lib/
│   ├── log.mjs          # 日志工具
│   └── commands/        # CLI 子命令实现
├── scripts/
